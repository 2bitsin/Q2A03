...
class ProcessCycles:
  ...
  def optimize_and_write(self, writer, table, cycle0):
    flat_table = {}
        ...
        for target, q_values in q_actions.items():
          for condition, value in q_values.items():
            ...
            if flat_action not in flat_table:
              flat_table[flat_action] = []
            flat_table[flat_action].append(flat_condition)
  ...
  def main(self):
    ...
    self.optimize_and_write(writer, cycle_table, am_data['*'])
    return 0
